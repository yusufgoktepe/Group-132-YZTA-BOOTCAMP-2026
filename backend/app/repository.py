...
EDUCATION_UNIVERSITY_ALIASES = {
    "osym-ankara-universitesi": "yok-ankara",
    "osym-bogazici-universitesi": "yok-bogazici",
    "osym-hacettepe-universitesi": "yok-hacettepe",
    "osym-istanbul-teknik-universitesi": "yok-itu",
    "osym-izmir-yuksek-teknoloji-enstitusu": "yok-iyte",
    "osym-yildiz-teknik-universitesi": "yok-yildiz",
}
...
def _event_row_to_dict(row: sqlite3.Row) -> dict[str, Any]:
    """V2 istemcileri için saat/konum alanlarını V3 verisinden de üretir."""
    event = dict(row)
    starts_at = event.get("starts_at") or ""
    if not event.get("time") and len(starts_at) >= 16:
        event["time"] = starts_at[11:16]
    if not event.get("location"):
        event["location"] = (
            event.get("location_name")
            or event.get("location_type")
            or event.get("participation_mode")
            or "Belirlenecek"
        )
    return event
...
def list_feed_candidates(
    ...
    university_alias = EDUCATION_UNIVERSITY_ALIASES.get(university_id, university_id)
    normalized_program_name = profile.get("program_name", "").casefold()
    program_alias = next(
        (
            alias
            for name, alias in PROGRAM_NAME_ALIASES.items()
            if name in normalized_program_name
        ),
        program_id,
    )
    ...
    rows = connection.execute(
        ...
        WHERE events.status = 'published'
          ...
          AND (
              events.university_id IN (?, ?) OR events.university_id = '' OR
              events.participation_mode IN ('online', 'hybrid')
          )
          AND (
              events.target_program_ids = '[]' OR
              events.target_program_ids LIKE ? OR events.target_program_ids LIKE ?
          )
