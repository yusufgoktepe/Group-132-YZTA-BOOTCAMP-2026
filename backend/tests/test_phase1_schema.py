...
def test_fresh_database_contains_v3_tables_and_seed_is_idempotent():
    ...
    try:
        ...
        with (PROJECT_ROOT / "data" / "sample" / "events_sample.csv").open(
            encoding="utf-8-sig", newline=""
        ) as file:
            legacy_event_count = sum(1 for _ in csv.DictReader(file))
        expected_tiers = Counter(row["event_tier"] for row in _rows("events_v3.csv"))
        expected_tiers["official"] += legacy_event_count

        assert first_event_count == second_event_count == 250 + legacy_event_count
        assert tiers == dict(expected_tiers)
